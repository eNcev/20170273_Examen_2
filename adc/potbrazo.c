...
void initADC()
{
    ...
    // Configuracion de los pines PA0, PA1, PA2 y PA3 como entradas ...
    // (analogicas). Poner en baja los 4 bytes menos significativos ...
    // del registro GPIOA_CRL)

    GPIOA->CRL &= ~(0xFFFF << 0);

    // Se procede a apagar el modulo ADC (poner en baja el bit 0 del ...
    // registro ADC_CR2)
    ADC1->CR2 &= ~(0x1 << 0);

    // Se realiza la configuracion de los secuenciadores del modulo ADC ...
    ...
    // Se realiza la configuracion de los canales ADC0; 1; 2 y 3 con una velocidad ...
    // de conversion de 239.5 ciclos (registro ADC_SMPR2)
...
uint16_t leerPuerto(uint8_t channel)
{
    ...
    // En el registro ADC_SQR3, se configurara el canal a ser leido, segun la ...
    // variable de entrada
    ADC1->SQR3 &= ~(0x3FFFFFFF);
    ADC1->SQR3 |= (channel << 0);

    // Del registro ADC_CR2, se coloca en ALTA el bit 0 y 22 para iniciar la ...
    // de datos analogicos a digitales en los canales previamente configurados:
    // PA0, PA1, PA2, PA3
    ADC1->CR2 |= (1 << 0);
    ADC1->CR2 |= (1 << 22);

    // Se monitorea el bit 1 del registro ADC_SR que, al estar en ALTA, indica ...
    // que la conversion en el ADC se completo.
    while(!(ADC1->SR & (1 << 1)));

    // Se realiza la lectura del valor convertido en el canal "channel"
    uint16_t valorADC = ADC1->DR & 0xFFF;

    // Se coloca en BAJA el bit 1 del registro ADC_SR para indicar que la converion ...
    // fue exitosa
    ADC1->SR &= ~(1 << 1);
